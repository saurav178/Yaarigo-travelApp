...
export default function PastTripCard({ trip }: PastTripCardProps) {
  return (
    <div className="flex flex-col md:flex-row items-center md:items-start bg-white border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300 p-4 md:p-5">
      <div className="relative w-full md:w-64 h-48 overflow-hidden">
        <Image
          src={trip.image}
          alt={trip.title}
      ...
      <div className="w-full md:w-2/3 mt-4 md:mt-0 md:ml-6 flex flex-col justify-between">
        ...
        {trip.travelerImages && trip.travelerImages.length > 0 && (
          <div className="flex items-center mt-3">
            <div className="flex -space-x-2">
              {trip.travelerImages.map((img, i) => (
                <Image
