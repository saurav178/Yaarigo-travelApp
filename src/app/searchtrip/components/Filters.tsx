...
//   matchPercent: number;
//   scorePercent: number;
//   interest: string; // now built from likes pills: "Hiking, Beaches"
//   tripType: string;
//   foodPref: string;
// };
...
//   interest?: string; // comma separated from parent
...
//   // local core state (applied on Apply Filter)
...
//   // Likes / Interests (NEW: behaves like Language)
//   const [likesInput, setLikesInput] = useState<string>("");
//   const [likes, setLikes] = useState<string[]>([]);

//   // Trip type & food pref remain chip-based
...
//   // parse parent string like "Hiking, Beaches" → ["Hiking", "Beaches"]
...
//   // parse for chip-based multi-select
...
//   // 🔄 sync likes from interest string
//   useEffect(() => {
//     setLikes(parseCSV(interest));
//   }, [interest]);

//   // 🔄 sync tripType / foodPref chips
//   useEffect(() => {
...
//   // generic add/remove for Language
//   const addLang = () => {
...
//   // toggle helper for chips (trip type / food)
...
//     if (opt === allLabel) {
//       const isAllOnly =
//         prev.length === 0 || (prev.length === 1 && prev[0] === allLabel);
//       if (isAllOnly) return [];
//       return [allLabel];
//     }
...
//     // ⬇ new: interest built from likes pills
...
//         {/* NEW: Likes / Interests input (same style as Language) */}
//         <div>
//           <label className="block text-xs font-medium text-gray-500 mb-2">
//             Interests
//           </label>
...
export default function Filters({
  ...
  // NEW: numeric rating local state (0..5)
  const [localMinRating, setLocalMinRating] = useState<number>(minRating ?? 0);
  ...
  const onStarClick = (value: number) => {
    // toggle: click current selected star to clear
    if (value === localMinRating) {
      setLocalMinRating(0);
    } else {
  ...
  return (
    <div className="bg-white p-5 rounded-xl shadow w-full max-w-md mx-auto max-h-[90vh] flex flex-col">
      ...
      <div className="mt-4 space-y-4 overflow-y-auto pr-1 flex-1">
        ...
        {/* ---------- NEW STAR RATING (replaces old radio rating) ---------- */}
        <div className="mb-3">
          <button
            onClick={() => setRatingOpen((o) => !o)}
            aria-expanded={ratingOpen}
                ...
                >
                  <FaStar
                    className={`w-5 h-5 transition-colors ${
                      n <= localMinRating ? "text-amber-300" : "text-gray-300"
                    }`}
                  />
                </button>
              ))}
