...
import { useState } from "react";
import Image from "next/image";
// import { FaExclamationTriangle, FaHeart, FaCheckCircle, FaShieldAlt,  FaWallet } from "react-icons/fa";
// import { PiMedalDuotone } from "react-icons/pi";
// import { GoDotFill } from "react-icons/go";
import {
  FaMapMarkerAlt,
...
export default function Similar({ trips = SIMILAR_TRIPS_DEMO }: Props) {
  const [likedTrips, setLikedTrips] = useState<number[]>([]);
  const toggleLike = (id: number) =>
    setLikedTrips((p) => (p.includes(id) ? p.filter((x) => x !== id) : [...p, id]));
  ...
  const getCategoryStyle = (category: string) => {
    switch (category) {
      case "Travel Enthusiast": return { bg: "bg-blue-100 text-blue-800", avatarBg: "bg-blue-500 text-white" };
      case "Featured Trip Leader": return { bg: "bg-yellow-100 text-yellow-800", avatarBg: "bg-yellow-500 text-white" };
      case "Featured Trip Agency": return { bg: "bg-orange-100 text-orange-800", avatarBg: "bg-orange-500 text-white" };
      default: return { bg: "bg-gray-100 text-gray-800", avatarBg: "bg-gray-500 text-white" };
    }
  };
