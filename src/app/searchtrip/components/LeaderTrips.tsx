...
type Props = { 
  leaders: Leader[]; // Changed: No default value
};

export default function LeaderTrips({ leaders }: Props) {
  const [liked, setLiked] = useState<number[]>([]);
  const router = useRouter();
    ...
    return (
      <main className="w-full flex justify-center px-3 lg:px-0">
        <div className="w-full mx-auto text-center py-10">
          <p className="text-gray-500">No trip leaders available</p>
        </div>
      </main>
    );
  }
