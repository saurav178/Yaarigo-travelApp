  ...
  return (
    <>
      <style jsx global>{`
        ...
        .travio-char.disappear {
          animation: travioDisappear 0.6s ease-out forwards;
          animation-delay: calc(var(--index) * 0.1s);
        }
      `}</style>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <Link
            href="/"
            className={`text-xl font-bold tracking-tight travio-logo ${
              isHeroPage && !isScrolled
                ? "text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.6)]"
                : "text-[#1D4350]"
            }`}
  ...
  return (
    <span>
      {text.split("").map((char, index) => (
        <span
          key={index}
          className={`travio-char ${isVisible ? "appear" : "disappear"}`}
